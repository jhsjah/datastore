...
const CardForm = () => {
  ...
  const fetchCards = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/slider1`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      let response;
      if (isEditing) {
        response = await axios.put(`${API_BASE_URL}/slider1/update/${currentCardId}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        setCards(cards.map((card) => (card._id === currentCardId ? response.data : card)));
        setMessage("Card updated successfully!");
      } else {
        response = await axios.post(`${API_BASE_URL}/slider1/add`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  const handleDelete = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`${API_BASE_URL}/slider1/delete/${id}`);
